...
export async function POST(request: NextRequest) {
  try {
    ...
    const systemPrompt = `You are an expert in automata theory. Generate a Deterministic Finite Automaton (DFA) equivalent to the given Regular Expression
...
Guidelines:
- Create a valid DFA based on the Regular Expression
- Convert the given Regular Expression to equivalent valid DFA 
...
IMPORTANT: Only generate a DFA equivalent to the Regular Expression provided. Do not use any placeholder or example data. Base your generation solely on the regular expression given.`

    const { text } = await generateText({
      model: google("gemini-2.0-flash"),
      system: systemPrompt,
      prompt: `Generate a DFA equivalent to: ${prompt}`,
      temperature: 0.1,
    })
