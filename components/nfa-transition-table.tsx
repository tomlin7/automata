...
export function NFATransitionTable({
  states,
  alphabet,
  transitions,
  epsilonTransitions,
  ...
  acceptStates,
  isDFA = false,
}: NFATransitionTableProps) {
  const hasEpsilonTransitions = epsilonTransitions && Object.keys(epsilonTransitions).length > 0
            ...
            <thead className="bg-muted/50">
              <tr>
                ...
                {hasEpsilonTransitions && (
                  <th className="px-3 py-3 text-center text-xs sm:text-sm font-medium text-foreground uppercase tracking-wider">
                    ε
                  </th>
                )}
              </tr>
            ...
            <tbody className="bg-card divide-y divide-border">
              {states.map((state) => (
                <tr key={state} className="hover:bg-muted/30 transition-colors">
                  ...
                  {hasEpsilonTransitions && (
                    <td className="px-3 py-3 text-center text-sm sm:text-base text-foreground">
                      <div className="flex flex-wrap gap-1 justify-center">
                        {epsilonTransitions![state]?.length > 0 ? (
                          epsilonTransitions![state].map((target, index) => (
                            <span key={index} className="text-xs bg-secondary/20 px-1 py-0.5 rounded">
                              {target}
                            </span>
                          ))
                        ) : (
